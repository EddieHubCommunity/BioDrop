...
test("Guest user cannot access manage repos", async ({ browser }) => {
  // fixture: make sure user is not logged in
  const context = await logout(browser);
  const page = await context.newPage();
  await page.goto("/account/manage/repos");
  await expect(page).toHaveURL(/auth\/signin/);
});

test("Logged in user can access manage repos", async ({ browser }) => {
  // fixture: make sure user is logged in
  const context = await login(browser);
  const page = await context.newPage();
  await page.goto("/account/manage/repos");
  await expect(page).toHaveURL(/account\/manage\/repos/);
});
