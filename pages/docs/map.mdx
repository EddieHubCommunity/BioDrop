...
To display user markers on the map page during local development, you can run the automated tests. The automated tests will visit some user profiles and create location information in the database. This will allow the user markers to appear on the map. For more information on how to execute the automated tests, please refer to the [Automated Tests Doc](https://biodrop.eddiehub.io/docs/contributing/automated-tests). Alternatively, you can manually visit multiple user profiles to populate the database with location information.
