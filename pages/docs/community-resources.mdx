...
import {data} from "@config/community-resources.js";
...
<table>
  ...
  {
      data.map((item,index)=>{
        return <tr key={item.username+item.name+index}>
    <td><a href={`https://github.com/${item.username}`}>{item.name}</a></td>
      <td >{item.contribution}</td>
      <td >{item.date}</td>
      <td ><a href={item.resource.url}>{item.resource.name ? item.resource.name : item.resource.url}</a></td>
    </tr>
    })
  }
    </tbody>
