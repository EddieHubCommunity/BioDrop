...
import communityResource from "@config/community-resources.json";
...
<table>
  ...
  <tbody>
    {communityResource.data.map((item, index) => {
      return (
...
export default ({ children }) => {
  communityResource.data.sort((a, b) => new Date(b.date) - new Date(a.date));
  return (
    <DocsLayout
