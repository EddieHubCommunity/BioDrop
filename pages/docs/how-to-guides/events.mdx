...
3. This `json` file will contain one object for the event and must have these six fields: `isVirtual` and/or `isInPerson`, `color`, `name`, `description`, `date`, `url`, and `location` (this field will be required only when `isInPerson` is `true`). 
...
4. Now you can commit your file and create a Pull Request. For more details please see <Link href="/docs/how-to-guides/editing">Editing Guide</Link>
   ...
   export default ({ children }) => (<DocsLayout  title="LinkFree Events Documentation" section="Customising your Profile" name="Events"> {children}</DocsLayout>);
