...
3. This `json` file will contain one object for the event and must have six fields, `isVirtual` and/or `isInPerson`, `color`, `name`, `description`, `date`, `url`, and `location` (this field will be required only when `isInPerson` is true), which will look like this:
