...
3. This `json` file will contain one object for the event and must have these six fields: `isVirtual` and/or `isInPerson`, `color`, `name`, `description`, `date`, `url`, and `location`. It can also optionally include other fields. For instance: `userStatus` & `speakingTopic`. It will look like this:
...
| location               | false    | Address of the event venue                                  |
