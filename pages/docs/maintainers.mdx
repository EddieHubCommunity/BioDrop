...
2. You have a LinkFree Profile which contains all the sections referred to in the Full Profile Example: https://linkfree.io/docs/full-profile-example
...
4. You have a consistent track record of:
   i. reviewing Profile Pull Requests to the LinkFree Project. This includes:
...
You will be contacted via DM on Discord to invite you to become a maintainer on the LinkFree project.
...
- Assigning: once the label is `status: ready for dev` please assign to a contributor in accordance with the [Contributing Guide](https://github.com/EddieHubCommunity/LinkFree/blob/main/CONTRIBUTING.md).
...
_Note: Keep an eye on the `maintainers` channel on Discord where we make maintainer decisions._
...
- LinkFree and LinkedIn recommendation from Eddie
- Badge (coming soon)
- Free upgrade to LinkFree Premium (coming soon)
- Participation in monthly "Maintainers Lottery" (each month one maintainer is selected at random and can choose a reward):
  - 30 minute 121 with Eddie
  - social shoutout post (Twitter or LinkedIn)
  - swag (if available)
  - guest priority on Eddie's YouTube Livestream (Profile Reviews)
...
- [Contributing Guide](https://github.com/EddieHubCommunity/LinkFree/blob/main/CONTRIBUTING.md)
- [Reviewers Docs](/docs/contributing/reviewers)

export default ({ children }) => (
  <DocsLayout title="LinkFree Maintainers">{children}</DocsLayout>
);
