...
## Debugging LinkFree with VS Code
This documentation explains how you can debug your LinkFree frontend and backend code using the Visual Studio Code (VS Code) debugger.
...
<Alert
  ...
  message={
    <>
      <strong>NOTE: </strong>
        This debugging setup is for a local development environment and will not work if you are running LinkFree using Docker Compose in your local setup. Additionally, it requires MongoDB to be available and running.
    </>
  }
/>
...
1. If you don’t already have a `launch.json` file, create one in the `.vscode` directory at the root of the LinkFree project.
