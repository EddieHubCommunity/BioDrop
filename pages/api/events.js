...
export default async function handler(req, res) {
  ...
  const events = await Promise.all(
    userFolders.flatMap(async (folder) => {
      ...
      const eventFilesContent = Promise.all(
        eventFiles.flatMap(async (file) => {
          try {
            const filePath = path.join(eventsPath, file);
            const eventData = await memoizedReadAndParseJsonFile(filePath);
            return {
              ...eventData,
              username: folder,
            };
